...
UltimateGCC::UltimateGCC(socd::SocdType socd_type) {
    _socd_pair_count = 5;
    _socd_pairs = new socd::SocdPair[_socd_pair_count]{
        socd::SocdPair{&InputState::left,    &InputState::right,   socd_type},
        socd::SocdPair{ &InputState::down,   &InputState::up,      socd_type},
        socd::SocdPair{ &InputState::down,   &InputState::up_2,      socd_type},
        socd::SocdPair{ &InputState::c_left, &InputState::c_right, socd_type},
        socd::SocdPair{ &InputState::c_down, &InputState::c_up,    socd_type},
    };
}

UltimateSwitch::UltimateSwitch(socd::SocdType socd_type) {
    _socd_pair_count = 5;
    _socd_pairs = new socd::SocdPair[_socd_pair_count]{
        socd::SocdPair{&InputState::left,    &InputState::right,   socd_type},
        socd::SocdPair{ &InputState::down,   &InputState::up,      socd_type},
...
void UltimateGCC::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;
    outputs.x = inputs.x;
    outputs.y = inputs.y;
...
void UltimateSwitch::UpdateDigitalOutputs(InputState &inputs, OutputState &outputs) {
    outputs.a = inputs.a;
    outputs.b = inputs.b;
    outputs.x = inputs.x;
    outputs.y = inputs.y;
...
void UltimateGCC::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
    // Coordinate calculations to make modifier handling simpler.
    UpdateDirections(
        inputs.left,
        inputs.right,
        inputs.down,
        inputs.up || inputs.up_2,
        inputs.c_left,
        inputs.c_right,
        inputs.c_down,
        inputs.c_up,
...
void UltimateSwitch::UpdateAnalogOutputs(InputState &inputs, OutputState &outputs) {
    // Coordinate calculations to make modifier handling simpler.
    UpdateDirections(
        inputs.left,
        inputs.right,
    ...
    if (shield_2) {
        ...
        // Easier UpB OoS (with MY + Shield 2)
        if ((inputs.mod_y || inputs.mod_y_2) && directions.y == +1) {
            outputs.leftStickY = 128 + (directions.y * 27);         // Partial shield tilt up (still considered neutral) = 28 (GCC = 27).
            if(inputs.b) {
                outputs.leftStickX = 128 + (directions.x * 100);        // Most horizontal for UpB (max drift).
                outputs.leftStickY = 128 + (directions.y * 60);         // Medium vertical for UpB = 80 (GCC = 60).
    ...
    if (directions.vertical && directions.cx) {                     // Basically this, but regardless of modifier:
        outputs.rightStickY = (outputs.leftStickY);                 // https://youtu.be/uTYSgyca8cI?t=1000.
        if(inputs.mod_x || inputs.mod_x_2) {
            outputs.rightStickX = 128 + (directions.cx * 40);           // Allows this to also work with ModX. 
        }                                                           // Mirrors medium walk and MX Horizontal Attack = 50 (GCC = 40).
        if(inputs.mod_y || inputs.mod_y_2) {
            outputs.rightStickX = 128 + (directions.cx * 70);           // Allows this to also work with ModY on GamecubeBackend. 
        }                                                           // (GCC adapter scales C-Stick differertly from main stick, whereas SwitchPro does not).
    }
    // Disable C-Stick for Extended UpB                             // Prevents SideB/DownB if C-Left/Right/Down is 
